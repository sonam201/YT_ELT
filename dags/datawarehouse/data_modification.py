        ...
        if schema == 'staging':
            ...
            cur.execute(
                f""" INSERT INTO {schema}.{table} ("Video_ID", "video_Title", "Upload_Date", "Duration", "Video_Views", "Likes_Count", "Comments_Count")
                            VALUES (%(Video_ID)s), %(Video_Title)s, %(Upload_Date)s, %(Duration)s, %(Video_Type)s, %(Video_Views)s, %(Like_Count)s, %(Comments_count)s);""", row
            )
        conn.commit()
        logger.info(f"Inserted row for Video_ID: {row['video_id']}") 
    except Exception as e:
        logger.error(f"Error inserting row for Video_ID: {row['video_id']}")
        raise e   
